...
def main():
    interface_logger, model_logger, world_logger, animation_logger = set_up_loggers()

    world = World(world_logger)
    model = Model(world.n_actions, model_logger)
    interface = Interface(interface_logger)
    animation = Animation(animation_logger)
    ...
    q_model_animation_sensor = mp.Queue()
...
def set_up_loggers():
    ...
    animation_logger.setLevel(LOGGING_LEVEL)
    animation_logger_file_handler = FileHandler(
        os.path.join("log", f"{time_name}_animation.log")
    )
    animation_logger_file_handler.setLevel(LOGGING_LEVEL)
    animation_logger_file_handler.setFormatter(Formatter("%(message)s"))
    animation_logger.addHandler(animation_logger_file_handler)

    return interface_logger, model_logger, world_logger, animation_logger
